...
void BattleSimulation::solve() noexcept
{
    ...
    // local data
    std::vector<Dice6> red_dices{};
    std::vector<Dice6> blue_dices{};

    int round_involved_attacker_armies{ 3 };
    int round_involved_defender_armies{ 3 };

    // initializes lists of red and blue dices with as much dices as possible
    for (int i = 0; i < round_involved_attacker_armies; ++i)
        red_dices.push_back(Dice6(6, 1, 1));

    for (int i = 0; i < round_involved_defender_armies; ++i)
        blue_dices.push_back(Dice6(6, 1, 1));
    ...
    while (!this->_ends()) {
        ...
        // resizes the lists of red and blue dices accordingly
        red_dices.resize(round_involved_attacker_armies);  // notice: does nothing is size doesn't change
        blue_dices.resize(round_involved_defender_armies); // but removes last dices from lists otherwise

        // rolls all of them
        for (auto& dice : red_dices)
            dice.roll();
        for (auto& dice : blue_dices)
            dice.roll();

        // sorts lists of dices in descending order
        auto cmp_fn = [](const Dice6& left, const Dice6& right) { return left.value > right.value; };
        std::sort(red_dices.begin(), red_dices.end(), cmp_fn);
        std::sort(blue_dices.begin(), blue_dices.end(), cmp_fn);
        ...
        int blue_wins{ 0 };
        std::cout << "          winner:";
        for (std::tuple<Dice6&, Dice6&> cmp_elem : std::views::zip(red_dices, blue_dices)) {
            Dice6 red_dice{ std::get<0>(cmp_elem) };
            Dice6 blue_dice{ std::get<1>(cmp_elem) };
